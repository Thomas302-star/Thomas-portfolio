...
export function ServicesSection() {
  return (
    <section className="border-b border-zinc-300 bg-[#f5f5f2] px-5 py-20 text-[#0a0a0a] sm:px-8 sm:py-28 lg:px-12 lg:py-36">
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-10 lg:grid-cols-[220px_minmax(0,1fr)] lg:gap-20">
          <Reveal><p className="text-xs uppercase tracking-[0.2em] text-zinc-500">Capabilities</p></Reveal>
          <div>
            <Reveal>
            ...
            <div className="mt-14 grid border-t border-zinc-300 sm:grid-cols-2 sm:mt-16">
              {services.map((service, index) => {
                const Icon = service.icon;
                const isRightColumn = index % 2 === 1;
                return (
                  <Reveal key={service.number} delay={index * 0.05}>
                    <article className={`h-full border-b border-zinc-300 py-8 sm:px-8 sm:py-10 lg:py-12 ${isRightColumn ? "sm:border-l" : "sm:pl-0"} ${index >= 2 ? "sm:pb-0" : ""}`}>
                      <div className="flex items-start justify-between gap-6"><Icon aria-hidden="true" size={25} strokeWidth={1.4} /><span className="text-xs tracking-[0.2em] text-zinc-400">{service.number}</span></div>
